...
class KLLoss():
    ...
    def __call__(self, slow, slow_pre, middle, middle_pre, fast, fast_pre):
        ...
        for slow_item, slow_pre_item in zip(slow, slow_pre):
            slow_kl = F.kl_div(slow_item.softmax(dim=-1).log(), slow_pre_item.softmax(dim=-1), reduction='sum')
            loss += slow_kl
        for middle_item, middle_pre_item in zip(middle, middle_pre):
            middle_kl = F.kl_div(middle_item.softmax(dim=-1).log(), middle_pre_item.softmax(dim=-1), reduction='sum')
            loss += middle_kl
        for fast_item, fast_pre_item in zip(fast, fast_pre):
            fast_kl = F.kl_div(fast_item.softmax(dim=-1).log(), fast_pre_item.softmax(dim=-1), reduction='sum')
            loss += fast_kl
