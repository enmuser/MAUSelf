...
class MAUCell(nn.Module):
    ...
    def forward(self, T_t, S_t, t_att, s_att):
        ...
        for i in range(self.tau):
            current_t_att.append(t_att[i][0])
        current_t_att = torch.stack(current_t_att, dim=0)
        current_s_att = []
        for i in range(self.tau):
            current_s_att.append(s_att[i][0])
        current_s_att = torch.stack(current_s_att, dim=0)
        s_next = self.conv_s_next(S_t[0])
        t_next = self.conv_t_next(T_t[0])
        weights_list_s = []
        weights_list_t = []
        for i in range(self.tau):
            weights_list_s.append((s_att[i][0] * s_next).sum(dim=(1, 2, 3)) / math.sqrt(self.d))
        for i in range(self.tau):
            weights_list_t.append((t_att[i][0] * t_next).sum(dim=(1, 2, 3)) / math.sqrt(self.d))
        ...
        T_trend = current_t_att * weights_list_s
        T_trend = T_trend.sum(dim=0)
        t_att_gate = torch.sigmoid(t_next)
        T_fusion = T_t[0] * t_att_gate + (1 - t_att_gate) * T_trend
        T_concat = self.conv_t(T_fusion)

        S_trend = current_s_att * weights_list_t
        S_trend = S_trend.sum(dim=0)
        s_att_gate = torch.sigmoid(s_next)
        S_fusion = S_t[0] * s_att_gate + (1 - s_att_gate) * S_trend
        ...
        current_t_att_2 = []
        for i in range(self.tau):
            current_t_att_2.append(t_att[i][1])
        current_t_att_2 = torch.stack(current_t_att_2, dim=0)
        current_s_att_2 = []
        for i in range(self.tau):
            current_s_att_2.append(s_att[i][1])
        current_s_att_2 = torch.stack(current_s_att_2, dim=0)

        s_next_2 = self.conv_s_next_2(S_t[1])
        t_next_2 = self.conv_t_next_2(T_t[1])
        weights_list_s_2 = []
        weights_list_t_2 = []
        for i in range(self.tau):
            weights_list_s_2.append((s_att[i][1] * s_next_2).sum(dim=(1, 2, 3)) / math.sqrt(self.d))
        for i in range(self.tau):
            weights_list_t_2.append((t_att[i][1] * t_next_2).sum(dim=(1, 2, 3)) / math.sqrt(self.d))
        ...
        T_trend_2 = current_t_att_2 * weights_list_s_2
        T_trend_2 = T_trend_2.sum(dim=0)
        t_att_gate_2 = torch.sigmoid(t_next_2)
        T_fusion_2 = T_t[1] * t_att_gate_2 + (1 - t_att_gate_2) * T_trend_2
        T_concat_2 = self.conv_t_2(T_fusion_2)

        S_trend_2 = current_s_att_2 * weights_list_t_2
        S_trend_2 = S_trend_2.sum(dim=0)
        s_att_gate_2 = torch.sigmoid(s_next_2)
        S_fusion_2 = S_t[1] * s_att_gate_2 + (1 - s_att_gate_2) * S_trend_2
        ...
        current_t_att_3 = []
        for i in range(self.tau):
            current_t_att_3.append(t_att[i][2])
        current_t_att_3 = torch.stack(current_t_att_3, dim=0)
        current_s_att_3 = []
        for i in range(self.tau):
            current_s_att_3.append(s_att[i][2])
        current_s_att_3 = torch.stack(current_s_att_3, dim=0)

        s_next_3 = self.conv_s_next_3(S_t[2])
        t_next_3 = self.conv_t_next_3(T_t[2])
        weights_list_s_3 = []
        weights_list_t_3 = []
        for i in range(self.tau):
            weights_list_s_3.append((s_att[i][2] * s_next_3).sum(dim=(1, 2, 3)) / math.sqrt(self.d))
        for i in range(self.tau):
            weights_list_t_3.append((t_att[i][2] * t_next_3).sum(dim=(1, 2, 3)) / math.sqrt(self.d))
        ...
        T_trend_3 = current_t_att_3 * weights_list_s_3
        T_trend_3 = T_trend_3.sum(dim=0)
        t_att_gate_3 = torch.sigmoid(t_next_3)
        T_fusion_3 = T_t[2] * t_att_gate_3 + (1 - t_att_gate_3) * T_trend_3
        T_concat_3 = self.conv_t_3(T_fusion_3)

        S_trend_3 = current_s_att_3 * weights_list_t_3
        S_trend_3 = S_trend_3.sum(dim=0)
        s_att_gate_3 = torch.sigmoid(s_next_3)
        S_fusion_3 = S_t[2] * s_att_gate_3 + (1 - s_att_gate_3) * S_trend_3
        ...
        # T_new_3 = T_new_3 * T_new_3_gate + (1 - T_new_3_gate) * T_new_2_lower
        #
        ...
        # S_new_3 = S_new_3 * S_new_3_gate + (1 - S_new_3_gate) * S_new_2_lower

        T_new_2_gate = torch.sigmoid(T_new_2)
        T_new_3_gate = torch.sigmoid(T_new_3)

        T_new_2 = T_new_2 * T_new_2_gate + T_new * (1 - T_new_2_gate)
        T_new_3 = T_new_3 * T_new_3_gate + T_new_2 * (1 - T_new_3_gate)
        ...
        S_new_3 = S_new_3 * S_new_3_gate + S_new_2 * (1 - S_new_3_gate)

        T_new_return.append(T_new)
        T_new_return.append(T_new_2)
        T_new_return.append(T_new_3)
        S_new_return.append(S_new)
        S_new_return.append(S_new_2)
        S_new_return.append(S_new_3)
