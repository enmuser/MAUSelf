...
class encoder_self(nn.Module):
    def __init__(self, in_channel=64, num_hidden=64, height=16, width=16):
        super().__init__() # dim =512, nc = 3, nf = 64, extra_deep = False
    ...
    def forward(self, input_): # 16 * 3 * 64 * 64
        ...
        h2 = self.c2(h1) # h2 = 16 * 128 * 16 * 16
        h3 = self.c3(h2) # h3 = 16 * 256 * 8 * 8
        h4 = self.c3(h3)  # h3 = 16 * 256 * 8 * 8
        return [h1, h2, h3], h4 # 16 * 512 * 4 * 4,[16 * 64 * 32 * 32,16 * 128 * 16 * 16, 16 * 256 * 8 * 8]
...
class decoder_self(nn.Module):
    def __init__(self, in_channel=64, num_hidden=64, height=16, width=16):
        ...
        self.upc2 = nn.Sequential(
            nn.Conv2d(in_channel * 2, num_hidden, kernel_size=7, stride=1, padding=3, ),
            nn.LayerNorm([num_hidden, height, width])
        )
        self.upc3 = nn.Sequential(
            nn.Conv2d(in_channel * 2, num_hidden, kernel_size=5, stride=1, padding=2,),
            nn.LayerNorm([num_hidden, height, width])
        )
        self.upc4 = nn.Sequential(
            nn.Conv2d(in_channel * 2, num_hidden, kernel_size=3, stride=1, padding=1,),
            nn.LayerNorm([num_hidden, height, width])
        )
    ...
    def forward(self, input_):
        vec, skips = input_ # vec = 16 * 512 * 4 * 4, skips = [16 * 64 * 32 * 32, 16 * 128 * 16 * 16, 16 * 256 * 8 * 8]
        d1 = self.upc1(vec) # d1 = 16 * 256 * 8 * 8 <= 16 * 512 * 4 * 4 d1
        d2 = self.upc2(torch.cat([d1, skips[-3]], 1)) # d2 = 16 * 128 * 16 * 16 <= cat(16 * 256 * 8 * 8 , 16 * 256 * 8 * 8) = 16 * 512 * 8 * 8
        d3 = self.upc3(torch.cat([d2, skips[-2]], 1)) # d3 = 16 * 64 * 32 * 32  <= cat(16 * 128 * 16 * 16, 16 * 128 * 16 * 16) = 16 * 256 * 16 * 16
        output = self.upc4(torch.cat([d3, skips[-1]], 1)) # output = 16 * 3 * 64 * 64  <= cat(16 * 64 * 32 * 32, 16 * 64 * 32 * 32) = 16 * 128 * 32 * 32
        return output, [d1, d2, d3] # 16 * 3 * 64 * 64,[16 * 256 * 8 * 8,16 * 128 * 16 * 16,16 * 64 * 32 * 32]
