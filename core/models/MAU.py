...
class RNN(nn.Module):
    def __init__(self, num_layers, num_hidden, configs):
        ...
        self.ex_encoders = enc_dec_models.encoder_self()
        self.ex_decoders = enc_dec_models.decoder_self()
    ...
    def forward(self, frames, mask_true):
        ...
        for t in range(self.configs.total_length - 1):
            ...
            for i in range(len(self.encoders)):
                ...
                frames_feature_input = self.encoders[i](frames_feature_input)
                frames_feature_encoded.append(frames_feature_input)
            ...
            if t == 0:
                ...
                for i in range(self.num_layers):
                    zeros = []
                    zeros.append(torch.zeros([batch_size, self.num_hidden[i], height, width]).to(self.configs.device))
                    zeros.append(torch.zeros([batch_size, self.num_hidden[i], height, width]).to(self.configs.device))# 16 * 64 * 16 * 16
                    zeros.append(torch.zeros([batch_size, self.num_hidden[i], height, width]).to(self.configs.device))
            ...
            for i in range(len(self.decoders)):
                # 1. 16 * 64 * 16 * 16 -> 16 * 64 * 32 * 32
                # 2. 16 * 64 * 32 * 32 -> 16 * 64 * 64 * 64
                pred_output = self.decoders[i](pred_output)
                if self.configs.model_mode == 'recall':
                    pred_output = pred_output + frames_feature_encoded[-2 - i]
    ...
    def _get_decoder_inputs(self, pred_feats, residuals): # residuals输入的特征=[16 * 64 * 32 * 32, 16 * 128 * 16 * 16, 16 * 256 * 8 * 8, 16 * 512 * 4 * 4]
        ...
        return [dec_input_feats[0], dec_input_feats[1:]] #
